...
- [x] M37a 招标文件分章解析：纯规则章节识别（目录锚定+标题键匹配），需求/评分/格式/须知章按用途全文取材，替代头部 22000 字截断
- [x] M37b Word/PDF 排版达标：docx 封面页、目录域、页眉页脚页码、宋体/黑体样式；PDF 封面页、目录页（虚线引导+实页码回填）、一级章节起新页、每页页码
- [x] M39 确定性废标点校验：金额大小写互验、跨表总报价一致、工期/投标有效期与招标要求比对，纳入提交前总检
- [ ] M38 格式文书引擎（开发中）：从招标文件「格式」章提取投标函/授权书/报价表等文书模板，字段自动填充+逐份复核+排入商务标
