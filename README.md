...
> AI 标书写作系统。从招标文件到投标技术方案初稿，一条主链路走完：**文档解析 → AI 生成目录 → AI 生成正文 → 导出 Markdown/Word/PDF → 电子盖章**。
>
...
- **主链路工作台**：上传招标文件 → 识别/选择标段分包 → 解析关键项 → 生成目录 → 设定全局事实 → 生成正文 → 导出 Markdown/Word/PDF → 电子盖章。
...
- [ ] 后续：订单发票、套餐续费提醒、具体微信/支付宝/Stripe SDK 接入、桌面图标格式、Win/Mac 签名公证、自动更新发布流水线、更多投标文件模板与质量规则

至此主链路（上传解析 → 选择投标范围 → 生成目录 → 生成正文 → 导出 Markdown/Word/PDF → 电子盖章）已打通。
