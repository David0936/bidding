...
> AI 标书写作系统。从招标文件到投标技术方案初稿，一条主链路走完：**文档解析 → 行业识别 → 响应矩阵 → 资料补齐 → AI 生成正文 → 导出 Markdown/Word/PDF → 电子盖章**。
>
...
- **主链路工作台**：上传招标文件 → 识别/选择标段分包 → 解析关键项 → 行业/采购类型识别 → 生成目录 → 设定全局事实 → 响应矩阵 → 资料补齐 → 生成正文 → 导出 Markdown/Word/PDF → 电子盖章。
...
至此主链路（上传解析 → 选择投标范围 → 行业识别 → 响应矩阵 → 资料补齐 → 生成正文 → 导出 Markdown/Word/PDF → 电子盖章）已打通。
