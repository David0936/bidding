...
- **三套目录方案**：目录阶段一次生成 3 套不同侧重点的标书目录，客户选择后可继续调整章节结构和每节预计字数。
...
- **证照扫描与插图**：营业执照、资质证书等照片上传时可自动「扫描增强」（黑白提亮，接近扫描件效果），并一键插入正文对应章节，导出 Word/PDF 时自动嵌入图片。
- **财务表格式化**：上传 xlsx/csv 财务报表或清单，自动解析为规范表格插入对应章节，导出时渲染为带边框的 Word/PDF 表格。
- **技术标/商务标分册**：目录章节可标记分册归属（未标记按标题关键词自动归类），一键按技术标、商务标、价格标拆分导出，或把多份 PDF 合并为一册。
- **富文本编辑**：正文编辑器带格式工具栏与实时预览，表格、证照图片按最终导出效果渲染。
...
- **项目管理与记录留痕**：项目管理支持按项目类型、创建时间和名称筛选，查重完成后自动保存历史摘要，方便复盘与定位风险。
- **字数包 + 会员计费**：内置 20万字、50万字、100万字、600万字、1000万字、3000万字、5000万字等个人/企业字数包，叠加试用/基础/VIP/企业套餐、功能权限、项目数上限、充值/消费流水和 AI 额度扣点。
- **代理人推广**：支持个人/企业代理人资料申请、邀请码生成、客户线索登记和待结算佣金估算，适合线下销售与老客户转介绍。
...
EASY_BIDDING_WORDS_PER_CREDIT=1000          # 前端展示口径：1 点额度约等于多少字
...
当前建议运营方式：客户在「额度中心」选择字数包并提交充值申请 → 线下公对公转账 → 管理员进入「管理员后台」→ 选择客户账户 → 开通试用/基础/VIP/企业套餐 → 设置到期日、项目数上限和功能权限 → 确认到账并手工分配额度。客户侧看不到模型供应商、Base URL、模型名或 API Key 状态。
...
- [x] M28 商用字数包：后端维护个人/企业字数包价格目录，客户侧可选择 20万字到 5000万字套餐并生成线下付款申请
- [x] M29 多方案目录与字数预算：目录阶段一次生成 3 套方案，支持选择方案、编辑目录、配置叶子章节预计字数，并在正文生成时按字数预算控制篇幅
- [x] M30 项目管理与查重记录：项目列表支持搜索、项目类型和创建时间筛选，查重完成后自动保存历史摘要
- [x] M31 代理人推广 MVP：个人/企业代理人资料申请、邀请码、线索登记、充值金额与待结算佣金估算
- [x] M32 表格/图片导出链路：正文 Markdown 表格渲染为 Word/PDF 真表格；material:// 图片引用嵌入 Word/PDF
- [x] M33 证照/照片资料：资料清单支持 png/jpg 上传，浏览器端「扫描增强」（灰度+对比度拉伸），一键插入对应章节
- [x] M34 财务表格式化：上传 xlsx/csv 自动解析为规范表格，插入财务对应章节，导出时渲染为 Word/PDF 表格
- [x] M35 分册拆分与合并：目录章节标记技术标/商务标/价格标（未标记按标题自动归类），按分册导出 Word/PDF/MD；多份 PDF 合并为一册
- [x] M36 富文本编辑体验：正文编辑器格式工具栏（加粗/小标题/列表/表格）+ 实时预览（表格与证照图片按导出效果渲染）
- [ ] 后续：订单发票、续费自动通知、具体微信/支付宝/Stripe SDK 接入、桌面图标格式、Win/Mac 签名公证、自动更新发布流水线、Word 模板中心、扫描件 OCR 文字识别与更多质量规则
